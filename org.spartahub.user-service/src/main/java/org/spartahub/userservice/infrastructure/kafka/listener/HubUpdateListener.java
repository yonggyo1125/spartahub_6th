...
public class HubUpdateListener {
    ...
    @Transactional
    @IdempotentConsumer("hub-updated")
    @KafkaListener(topics = "${topics.hub.updated}", groupId = "user-group")
    public void onUpdated(Message<String> message, Acknowledgment ack) {
        try {
            Hub hub = JsonUtil.fromJson(message.getPayload(), Hub.class);
            if (hub != null) {
                adminService.syncHubInfo(hub);
                log.info("허브 업데이트 처리 완료: hubId={}", hub.getId());
            }
            ack.acknowledge();
        } catch (Exception e) {
            log.error("허브 업데이트 처리 실패:{}", e.getMessage(), e);
            throw e;
        }
    }
